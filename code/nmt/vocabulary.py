...
class Vocabulary(object):
    def __init__(self, word_seq, max_vocab_size):
        ...
        self.vocab = dist.most_common(max_vocab_size)

        # Creating an array of words from the vocabulary set,
        # we will use this array as index-to-word dictionary
        self.ix_to_word = [word[0] for word in self.vocab]
        # ADD special vocabulary symbols at the start
        self._insert_symbol_to_vocab(self.ix_to_word, SpecialSymbols.PAD, SpecialSymbols.PAD_ID)
        self._insert_symbol_to_vocab(self.ix_to_word, SpecialSymbols.GO, SpecialSymbols.GO_ID)
        self._insert_symbol_to_vocab(self.ix_to_word, SpecialSymbols.EOS, SpecialSymbols.EOS_ID)
        self._insert_symbol_to_vocab(self.ix_to_word, SpecialSymbols.UNK, SpecialSymbols.UNK_ID)
