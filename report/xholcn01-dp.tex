...
\abstract[cs]{Cílem této práce je popsat a vytvořit systém pro strojový překlad textu postavený na rekurentních neuronových sítích. K~tomu je použita architektura enkodér-dekodér umožňující překlad po celých větách. Výsledkem je knihovna \emph{nmt}, určená k~provádění experimentů s~různými parametry modelu. Jejich výsledky jsou porovnány vůči systému postavenému na nástroji pro statistický překlad Moses.}
\abstract[en]{The goal of this thesis is to describe and build a system for neural machine translation. System is built with recurrent neural networks -- encoder-decoder architecture in particular. The result is a \emph{nmt} library used to conduct experiments with different model parameters. Results of the experiments are compared with system built with the statistical tool Moses.}
